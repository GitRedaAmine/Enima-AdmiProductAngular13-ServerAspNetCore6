        ...
        catchError((error: HttpErrorResponse) => {
          ...
          if (error.status === 401 && session != null && !this.tokenService.isLoggedIn() && !this.isRefreshingToken) {
           ...
           // console.log('Access Token is expired, we need to renew it');
            this.toastr.warning('Access Token is expired, we need to renew it',"refreshToken")
            this.userService.refreshToken(session).subscribe({
              next: data => {
              //  console.info('Tokens renewed, we will save them into the local storage');
                this.toastr.success('Tokens renewed, we will save them into the local storage',"refreshToken")
            ...
            this.toastr.error('the refresh token has expired, the user must login again',"refreshToken")
